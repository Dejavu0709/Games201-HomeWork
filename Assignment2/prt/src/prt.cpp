        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3d dirDouble = dir.cast<double>();
                    int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);

                    for(int l = 0; l < (int)SHOrder + 1; l++)
                    {
                        for(int m = -l; m <= l; m++)
                        {
                            float dt = CalcArea(x,y,width,height);
                            double sh = sh::EvalSH(l, m, dirDouble);
                            SHCoeffiecents[sh::GetIndex(l,m)] += Le * sh * dt;
                        }
                    }
                }
            }
